...
builder.Services.AddSingleton<IExchangeRateConversionService, ExchangeRateConversionService>();
...
{
    var rate = service.GetRate(from, to, date, source, frequency);
    ...
    public class InMemoryExchangeRateDataStore : IExchangeRateDataStore
    {
        ...
        public Task SaveExchangeRatesAsync(IEnumerable<ExchangeRate.Core.Entities.ExchangeRate> rates)
        {
            foreach (var rate in rates)
            {
                ...
                else if (existingRate.Rate != rate.Rate)
                {
